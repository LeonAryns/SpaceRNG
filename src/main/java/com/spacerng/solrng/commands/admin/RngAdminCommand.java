...
public class RngAdminCommand implements CommandExecutor, TabCompleter {
    ...
    private static final List<String> SUBCOMMANDS = List.of(
            ...
            "milestones", "farmfill", "boost", "nova", "placeholders", "payout", "crate", "tophead", "floatingitem", "boss", "pet", "discord", "advancements", "shiny", "firsts", "lorestyles", "tagstyles", "menustyles", "hoestyles", "standingstyles", "enchantstyles", "novastyles", "auratest", "holo", "help");
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (sub.equals("pet")) {
            ...
            if (args.length == 3) {
                List<String> ids = new ArrayList<>(plugin.getPetManager().getTypes().keySet());
                ids.add("all");
                return partial(args[2], ids);
            }
            if (args.length == 4) return partial(args[3], playerNames());
            return List.of();
        }
        if (sub.equals("boss")) {
