...
public class SkillTreeManager {
    ...
    public void load(FileConfiguration config) {
        ...
        for (String id : section.getKeys(false)) {
            ...
            try {
                ...
                nodes.put(id, new SkillNode(id, display, moneyCost, requires, effect, value, maxLevel));
            } catch (Exception ex) {
    ...
    public boolean canAfford(Player player, SkillNode node) {
        Economy economy = economy();
        return economy != null && economy.getBalance(player) >= node.getMoneyCost();
    }

    /**
     * Attempts to purchase a node, paying with Money. For a leveled node
     * (maxLevel > 1) this buys the NEXT level (same cost each time) instead
     * of a one-time unlock. Returns true on success.
     */
    public boolean purchase(Player player, PlayerData data, String nodeId) {
        ...
        Economy economy = economy();
        if (economy == null || economy.getBalance(player) < node.getMoneyCost()) return false;
        economy.withdrawPlayer(player, node.getMoneyCost());
